...
class AudioDataset(Dataset):
    def __init__(self,
        ...
        """
        assert batch_type in ['static', 'dynamic'] or 'sort_' in batch_type
        data = []
        ...
        elif 'sort_' in batch_type:
            random.shuffle(data)
            sort_size = int(batch_type.split('_')[-1])
            for block_start in range(0, num_data, sort_size):
                data_block = data[block_start:block_start + sort_size]
                data_block.sort(key=lambda x: -x[2])
                for i in range(0, len(data_block), batch_size):
                    batch = data_block[i:i + batch_size]
                    self.minibatch.append([(data[0], data[1], data[3]) for data in batch])
        else:
            print("batch type error!")
